...
本書で作った検索システムの公開版は、[yuyusearch.subcatalog.net](https://yuyusearch.subcatalog.net/)で動かしています。実作品については検索結果の巻・頁・コマ番号を返し、人手で確認した発言だけ話者名も表示します。セリフ本文と画像は返しません。

## 本との対応

このリポジトリは、C108で発行した技術同人誌『セリフはわかった。で、誰が言った？』の再現用リポジトリです。本で扱った工程のうち、手元のページ画像から構造化JSONを作り、話者を付け、検索DBとローカル検索画面を作るところまでを実行できます。

本で使用した作品画像、抽出済みセリフ、人物名簿、評価用goldは収録していません。本文では、抽出と話者推定の精度を人手で作ったgoldにより検証し、複数VLMとの比較や評価をやり直した経緯まで説明しています。
...
[公開検索サイト](https://yuyusearch.subcatalog.net/)は、この標準公開モードに加えて、人手で確認済みの発言だけ話者名を表示する専用の索引を使っています。VLMが推定しただけの人物名は公開しません。
